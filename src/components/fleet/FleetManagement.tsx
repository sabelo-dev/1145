...
import {
  Car, Users, Wrench, Plus, CheckCircle, Truck, BarChart3,
} from "lucide-react";
...
const FleetManagement: React.FC = () => {
  ...
  const [newVehicle, setNewVehicle] = useState({
    license_plate: "", make: "", model: "", year: "", color: "", fleet_id: "",
  });
  ...
  const addVehicle = async () => {
    if (!newVehicle.license_plate.trim() || !newVehicle.make.trim() || !newVehicle.year) return;
    ...
    if (error) {
      ...
      setNewVehicle({ license_plate: "", make: "", model: "", year: "", color: "", fleet_id: "" });
  ...
  const assignDriver = async (vehicleId: string, driverId: string | null) => {
    ...
    if (!error) { toast({ title: "Driver Assigned" }); fetchAll(); }
  };
      ...
      <Tabs defaultValue="vehicles">
            ...
            <Dialog open={showAddVehicle} onOpenChange={setShowAddVehicle}>
              ...
              <DialogContent>
                ...
                <div className="space-y-4">
                  <div><Label>License Plate *</Label><Input value={newVehicle.license_plate} onChange={(e) => setNewVehicle({ ...newVehicle, license_plate: e.target.value })} /></div>
                ...
                <Card key={vehicle.id}>
                  <CardContent className="pt-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={vehicle.driver_id || "none"} onValueChange={(v) => assignDriver(vehicle.id, v === "none" ? null : v)}>
                    ...
                    <div className="flex gap-2 mt-3">
                      {vehicle.status !== "active" && <Button size="sm" variant="outline" onClick={() => updateVehicleStatus(vehicle.id, "active")}><CheckCircle className="h-3 w-3 mr-1" />Activate</Button>}
                      {vehicle.status === "active" && <Button size="sm" variant="outline" onClick={() => updateVehicleStatus(vehicle.id, "maintenance")}><Wrench className="h-3 w-3 mr-1" />Maintenance</Button>}
                    </div>
                  </CardContent>
                </Card>
              ))}
        ...
        <TabsContent value="fleets" className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <Dialog open={showAddFleet} onOpenChange={setShowAddFleet}>
              <DialogTrigger asChild><Button><Plus className="h-4 w-4 mr-2" />Create Fleet</Button></DialogTrigger>
              <DialogContent>
                  ...
                  <Card key={fleet.id}>
                    <CardContent className="pt-6">
                      ...
                      <div className="flex gap-4 mt-3 text-sm text-muted-foreground">
                        ...
                        <span className="flex items-center gap-1"><CheckCircle className="h-4 w-4 text-primary" />{fleetVehicles.filter((v) => v.status === "active").length} active</span>
                      </div>
                    </CardContent>
                  </Card>
                );
        ...
        <TabsContent value="overview" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Active</CardTitle><CheckCircle className="h-4 w-4 text-primary" /></CardHeader>
              <CardContent><div className="text-2xl font-bold">{vehicles.filter((v) => v.status === "active").length}</div></CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">In Maintenance</CardTitle><Wrench className="h-4 w-4 text-muted-foreground" /></CardHeader>
