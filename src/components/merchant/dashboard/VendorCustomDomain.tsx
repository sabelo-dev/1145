...
const VendorCustomDomain = () => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "verifying": return <RefreshCcw className="h-5 w-5 text-gold/100 animate-spin" />;
      ...
      {domains.length === 0 ? (
        ...
        domains.map((domain) => (
          <Card key={domain.id}>
            ...
            {domain.status === "verifying" && (
              <CardContent>
                <div className="p-4 bg-gold/100/10 border border-gold/100/20 rounded-lg flex items-center gap-3">
